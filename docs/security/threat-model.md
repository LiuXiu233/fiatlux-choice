...
| --- | --- | --- | --- |
| 账户接管 | 弱密码、会话窃取、TOTP/恢复码重放 | Argon2、HttpOnly/Secure/SameSite=Strict cookie、owner/admin 强制 TOTP、密码后短时挑战、5 次尝试、TOTP counter 防重放、恢复码一次性、会话过期、登录限流、TLS、审计 | 验证器/恢复码与密码同设备失陷仍可被接管；极小团队必须保留离线 owner 恢复与设备安全；产品负责人 |
| MFA 密钥泄漏或轮换失误 | 数据库密文与应用 key 同时泄漏；直接替换 key 导致全员无法验证 | 独立 AES-256-GCM key、随机 IV/AAD key ID、API 日志不记 seed/代码、恢复码只存哈希、生产环境失败关闭、离线恢复需双固定确认 | 当前仅支持单活动 key ID，没有在线重加密；轮换前须实现受控迁移或批准逐用户重登记；信息安全负责人 |
