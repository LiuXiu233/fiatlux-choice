...
- session cookie 使用 `Secure`、`HttpOnly`、`SameSite=Strict`；登录与高风险接口限流。owner/admin 默认强制 TOTP MFA：密码通过后只发短时 HttpOnly 挑战，TOTP 允许有限时钟偏差并阻止计数器重放，恢复码高熵且一次性，正式会话只在第二因素通过后创建。
- TOTP seed 使用独立 32-byte Base64url key 做 AES-256-GCM 加密，AAD 绑定 key ID；恢复码数据库只存 SHA-256。`MFA_ENCRYPTION_KEY` 不得复用 `SESSION_SECRET`、进入 Git/日志或在已有登记用户时直接替换。
...
| MFA_ENCRYPTION_KEY | 预先制定轮换窗口；当前单活动 key ID | TOTP 密钥或数据库/主机疑似泄漏 |
| PostgreSQL/MinIO | 至少每年 | 配置或主机泄漏、运维离职 |
| LLM/GitHub token | 90 天或供应商更短要求 | 权限扩大、日志泄漏、人员变更 |
| age 身份 | 按离线托管制度 | 私钥副本丢失或未授权访问 |
| 备份 Ed25519 签名密钥 | 至少每年并按归档保留期保留旧公钥 | 在线私钥、主机或批准指纹渠道疑似失陷 |
| Caddy 内部 CA | 到期前计划轮换 | 根私钥疑似泄漏 |

轮换前确认依赖方和恢复链路；轮换后撤销旧密钥并实际验证。仅修改环境文件但未重建容器，不算完成轮换。当前 MFA 运行时只接受一个 key ID，直接替换会令既有 TOTP 密钥不可解密；必须先实现并验证受控重加密，或经批准逐用户重新登记，不得盲目改环境值。备份签名密钥轮换必须保存新旧指纹、生效时间和各自覆盖的归档范围；旧公钥在旧归档保留期内不能删除，私钥泄露后也不能仅凭新密钥把旧签名重新判为可信。MinIO 优先固定 access ID 轮换 secret；若更换 ID，bootstrap 不会自动撤销旧用户，必须按运维手册以 root-only 一次性操作显式禁用/删除，并用旧凭据做失败验证。
