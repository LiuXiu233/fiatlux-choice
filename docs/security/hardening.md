...
轮换前确认依赖方和恢复链路；轮换后撤销旧密钥并实际验证。仅修改环境文件但未重建容器，不算完成轮换。备份签名密钥轮换必须保存新旧指纹、生效时间和各自覆盖的归档范围；旧公钥在旧归档保留期内不能删除，私钥泄露后也不能仅凭新密钥把旧签名重新判为可信。MinIO 优先固定 access ID 轮换 secret；若更换 ID，bootstrap 不会自动撤销旧用户，必须按运维手册以 root-only 一次性操作显式禁用/删除，并用旧凭据做失败验证。
