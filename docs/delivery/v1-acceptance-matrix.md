...
| 不可变运行异常增量本地通过 | `cbc3d92ed18563138e60fe2e449b24ba39a118f4` 已完成运行异常人工闭环、exact-SHA 源码/数据库回归和生产构建；冻结前候选 Compose/镜像结果单独分列，不冒充最终 SHA 重建或外部门禁 |
...
本矩阵严格区分代码存在、不可变实现提交本地测试、本地恢复、本地 synthetic bridge 演练、GitHub CI、耀光目标办公内网、真实设备和真实外部集成。`101d2f0…` 的本机 Compose 不是广州办公内网生产部署；本地 arm64 image ID 也不是 GHCR 双平台 root digest。`cbc3d92…` 的运行异常证据还明确区分冻结前候选 Compose/镜像与最终收紧损坏审计判定后的 exact-SHA 回归。后续证据文档提交只记录实现 SHA，最终发布仍须统一 Git/GitHub/GHCR 与目标环境身份；旧 v1 恢复和同内容标签升级数字不能滚入当前通过口径，synthetic bridge 也不能冒充历史生产 N−1。
...
| 运行异常人工闭环 | `cbc3d92…` 按组织派生 advisor/workflow/backup `lease_expired`，owner/admin 以证据、补偿引用和禁止重放确认追加处置；原失败记录、队列和执行器不变，同一事件并发只成功一次 | 不可变运行异常增量本地通过 | 目标内网用真实责任人分别演练顾问、工作流和备份；长期未处置告警仍待运营验收 |
...
| lease-expired 人工处置 | `cbc3d92…` 的组织隔离列表、RBAC、损坏审计失败关闭、partial 强制补偿、事件 advisory lock、单次追加审计和无重放均由 PostgreSQL 覆盖；冻结前真实 Compose 已完成一次 workflow checkpoint 补偿并经整体重启读取 | 不可变运行异常增量本地通过 | 最终 SHA/目标内网对 advisor、workflow、backup 三类逐项复演；既有 admin 权限须经批准的 `system-role-maintenance` 补齐 |
...
| 层级 | 当前发布状态 | 最终证据要求 |
| --- | --- | --- |
| Biome / ShellCheck / Actionlint / 类型 | **不可变基线与最新增量本地通过** | `cbc3d92…` exact SHA：Biome 217 个文件、ShellCheck、7 项类型和 Actionlint 1.7.12 通过；GitHub CI 复现 |
| 单元/聚合 | **不可变运行异常增量本地通过** | `cbc3d92…` exact SHA：190/190 单元通过；GitHub CI 待复现 |
| API / worker / PostgreSQL / pg-boss | **不可变基线与最新增量本地通过** | `cbc3d92…` exact SHA：临时 PostgreSQL 应用 11 migrations 后 API 19 files/93 tests、worker 5 files/29 tests；运行异常覆盖租户隔离、并发一次关闭、partial 强制补偿、损坏处置审计失败关闭和原运行不变 |
| MinIO / S3 | **不可变运行异常增量继承未变层，有 exact-SHA 外部边界** | 冻结前候选真实字节/错误清理 2/2，独立 Compose 的 app/backup/restore 最小权限通过；最终冻结只改 API 损坏审计选择且未改存储层，但未在 `cbc3d92…` 重跑真实 MinIO |
| Web / Playwright / PWA | **不可变运行异常增量本地通过，有真机边界** | 前端冻结后未再变化：mock 48 passed/6 条件 skip；独立 1440×1000/390×844 浏览器 0 溢出/0 意外错误；Compose SW 11 条静态缓存、0 API/health 缓存、0 installability error，整体重启后仍可读处置 |
| 全 workspace / 镜像构建 | **不可变基线与最新增量分层通过** | `cbc3d92…` exact SHA production build 为运行异常块 11.13 KiB、教育块 128.18 KiB、主块 428.40 KiB、PWA 11 precache/724.12 KiB；冻结前候选重建 API/worker/Web 并启动新卷 Compose，最终 API 镜像未因后续损坏审计加固而重建；GHCR 双平台待发布 |
| 安全/供应链 | **不可变运行异常增量分层通过，有 GitHub 边界** | `cbc3d92…` exact SHA Gitleaks 通过；冻结前候选 Semgrep 10/10 canary/90 生产目标 0、`audit --prod` 0，API/worker/Web Trivy HIGH/CRITICAL 0、SPDX 204/225/176；最终冻结未增依赖但 API 镜像未重扫，GitHub CodeQL/security 待运行 |
...
| 验收项 | 当前证据 | 状态 | 最终闸门 |
| --- | --- | --- | --- |
| Compose 与迁移 | `101d2f0…` 完整基线；运行异常冻结前候选又在 `fiatlux-opsqa-20260720` 新项目/新卷完成五职责 bootstrap、38 表/11 迁移、pg-boss、73 条 seed、六服务 healthy、整体重启、部署/MinIO 权限和 workflow 处置；QA 资源均清理，最终收紧损坏审计后的 SHA 未重建 Compose | 不可变基线与运行异常增量分层通过 | 最终 SHA 的 GHCR/目标内网 Compose 复现 |
| HTTPS、PWA 与浏览器 | 基线与运营状态增量保持有效；运行异常候选另通过 1440×1000、390×844、SW 11 条静态缓存、0 API/health 缓存、0 installability error、无溢出及六服务重启后读取 | 不可变运行异常增量本地通过 | 目标 DNS、CA、防火墙和真实设备 |
...
| GitHub 交付 | 运行异常实现 `cbc3d92ed18563138e60fe2e449b24ba39a118f4` 已推送，Draft PR #12 保持开放；最新 GitHub run 在证据文档推送后只读核验，历史 run 均在 runner 前因账户付款失败或 spending limit 不足被阻断 | 部分完成 / 外部计费阻断 | 推送文档 head 后复核远端；修复 Billing & plans 后重跑并取得绿色 CI/security、制品和批准；再合并 |
...
| advisor/workflow/backup 后台任务 | 原子 claim、CAS、lease-expired 审计和 partial checkpoint 通过；`cbc3d92…` 增加组织隔离处置 API/UI，PostgreSQL 证明损坏审计不隐藏事项、并发仅一次追加且不重放，冻结前 Compose 完成 workflow checkpoint 人工补偿 | 目标分别演练真实顾问、工作流和备份命令/产物，建立长期未处置升级规则 |
...
当前仍只能称为**受控候选**。不可变基线 `101d2f0…` 已完成完整恢复层验收；最新运行异常实现 `cbc3d92…` 已完成 exact-SHA 源码/数据库/生产构建回归，并以冻结前候选完成独立 Compose、桌面/移动/PWA、实际 workflow 补偿和三镜像供应链复验，树差异与未重跑层已在[运行异常增量证据](./evidence/operational-incident-acceptance-cbc3d92-20260720.json)明示。恢复代码自基线未变，本增量没有伪称重跑恢复或最终 API 镜像。Draft PR、GitHub/GHCR 与目标环境身份仍未统一；GitHub CI/security 先前因账户计费在 runner 前被阻断，GHCR、历史生产 N−1/目标发布、生产恢复入口、目标办公内网、真机、真实 LLM/GitHub、73 条专业复核、残余风险决策和业务批准仍未完成。在这些门禁全部关闭前，不得宣布“V1 已完成”，也不得用于无人监督的生产关键操作。

## 9. 最终 SHA 与目标环境必须补录

| 字段 | 当前值 | 要求 |
| --- | --- | --- |
| 完整 Git SHA / tag | 完整恢复基线 `101d2f0938adfa0caa8ed576f6587a5c78ae74a5`；最新运行异常实现 `cbc3d92ed18563138e60fe2e449b24ba39a118f4` 已推送；证据文档提交与 tag 待创建 | 推送文档 head 后记录完整 SHA；确定 commit/tag 签名政策，生产发布记录受保护 tag |
| GitHub PR / CI / 安全 run | Draft PR #12；`cbc3d92…` 已推送，证据文档 head 待本轮提交；PR Checks 是远端状态权威来源；历史 run 均为账户付款/spending limit 在 runner 前阻断 | 修复 Billing & plans 后重跑，只有实际 step 执行且绿色才能关闭门禁 |
...
| 最终测试报告 | **`cbc3d92…` exact-SHA 源码/数据库/构建通过；Compose/镜像 final-freeze 差异已披露** | 记录证据文档 SHA 与 GitHub run；功能源码漂移或冻结最终制品时重跑全层 |
